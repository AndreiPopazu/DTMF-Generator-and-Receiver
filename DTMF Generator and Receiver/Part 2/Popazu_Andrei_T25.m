...
T=4;                            %#input %perioada semnalului
P=T/2;                                  %jumatate de perioada
s1=-2;                          %#input %valoarea inferioara a intervalului ramura 1
d1=-1;                          %#input %valoarea superioara a intervalului ramura 1
s2=-1;                          %#input %valoarea inferioara a intervalului ramura 2
d2=1;                           %#input %valoarea superioara a intervalului ramura 2
s3=1;                           %#input %valoarea inferioara a intervalului ramura 3
d3=2;                           %#input %valoarea superioara a intervalului ramura 3

%zona de definire a coeficientilor fourier
f1(t) = -(t^0);                 %#input %semnalul de intrare ramura 1
f2(t) = t;                      %#input %semnalul de intrare ramura 2
f3(t) = t^0;                    %#input %semnalul de intrare ramura 3
...
for n=1:1:10
    ...
    if (n == 2 || n == 4 || n == 7 || n == 10)
        ...
        plot(t1,-(t1.^0),'-k','LineWidth',1)%graficul semnalului de intrare ramura 1
        hold on
        plot(t2,t2,'-k','LineWidth',1)      %graficul semnalului de intrare ramura 2
        hold on
        plot(t3,t3.^0,'-k','LineWidth',1)   %graficul semnalului de intrare ramura 3
        hold on
